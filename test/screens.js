...
/* Every quest is one agreed amount — no unit switch, and the field is just
   "Price" rather than "Fixed price" (ADR-011). */
ok("no fixed-versus-hourly switch", !byText("Per hour") && !byText("Fixed price"));
...
ok("the price row needs no derivation", H.has(root, "Quest price") && !H.has(root, "an hour"));
